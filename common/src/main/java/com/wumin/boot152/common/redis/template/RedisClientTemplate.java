package com.wumin.boot152.common.redis.template;
...
public class RedisClientTemplate {
    private final Logger log = LoggerFactory.getLogger(RedisClientTemplate.class);
    @Autowired
    private RedisDataSource redisDataSource;
    public RedisClientTemplate(){
        super();
        log.info("redisClientTemplate创建完成");
    }
     ...
     * @return
     */
    public String set(String key, Object value, int... db) {
        ShardedJedis shardedJedis = null;
        String result = null;
        if (db.length > 0) {
            shardedJedis = redisDataSource.getRedisClient(db[0]);
        } else
        ...
        try {
            result = shardedJedis.set(key, JSONObject.toJSONString(value));
        } catch (Exception e) {
    ...
    public String setex(String key, Object value, int seconds, int... db) {
        ShardedJedis shardedJedis = null;
        String result = null;
        if (db.length > 0) {
            shardedJedis = redisDataSource.getRedisClient(db[0]);
        ...
        try {
            if (value instanceof Integer) {
                result = shardedJedis.setex(key, seconds, String.valueOf(value));
            } else
                result = shardedJedis.setex(key, seconds, JSONObject.toJSONString(value));
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            broken = true;
        } finally {
    ...
    public Long hdel(String key, String field, int... db) {
        Long result = null;
        ShardedJedis shardedJedis = getRedisClient(db);
        if (shardedJedis == null) {
            return result;
        }
        boolean broken = false;
    ...
    private ShardedJedis getRedisClient(int... db) {
        if (db.length > 0) {
            return redisDataSource.getRedisClient(db[0]);
        } else {
    ...
    public Long zadd(String key, double score, String member, int seconds, int... db) {
        Long result = null;
        ShardedJedis shardedJedis = null;
        if (db.length > 0) {
            shardedJedis = redisDataSource.getRedisClient(db[0]);
    ...
    public Set<String> zrange(String key, int start, int end, int... db) {
        Set<String> result = null;
        ShardedJedis shardedJedis = null;
        if (db.length > 0) {
            shardedJedis = redisDataSource.getRedisClient(db[0]);
    ...
    public Set<Tuple> zrangeWithScores(String key, int start, int end, int... db) {
